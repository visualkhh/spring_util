...
public @Data class MsgWaitJoinVO extends DefaultVO {
...
//	@JoinColumn(name="WAIT_ID",insertable=false,updatable=false)
	@JoinColumn(name="WAIT_ID",referencedColumnName="WAIT_ID",insertable=false,updatable=false)
//	@ElementCollection
//	@JoinColumn(name="WAIT_ID", referencedColumnName="WAIT_ID")
	List<MsgCompanyVO> companys = new ArrayList<MsgCompanyVO>();
...
//	@ElementCollection
//	@JoinColumn(name="WAIT_ID", referencedColumnName="WAIT_ID")
	List<MsgAreaVO> areas = new ArrayList<MsgAreaVO>();
	
	public MsgWaitJoinVO(){
		super();
		init();
	}
	...
	public void init(){
		...
		if(null==status){
			status = Code.CODE_MSG_STATUS_WAIT.cd();//WAIT
		}
		if(null==callback_type){
			callback_type = Code.CODE_CALLBACK_TYPE_TEXT.cd();//"TXT";
		}
	}
