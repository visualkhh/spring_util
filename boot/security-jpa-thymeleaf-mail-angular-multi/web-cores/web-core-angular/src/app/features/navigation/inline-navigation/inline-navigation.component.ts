...
import {I18nService, JsonApiService, UserService} from '@web-core/app/services';
...
@Component({
  selector: 'web-core-inline-navigation[url]',
  ...
  ngOnInit() {
    this.userService.user$.subscribe(it => {
      ...
      const data = this.userService.authDupDepthDetailsLoopFilter(
          sit => 'GET' === sit.crudTypeCd && (sit.url === this.url || ('USE001' === sit.regexpCd && RegExp(sit.url).test(this.url))),
