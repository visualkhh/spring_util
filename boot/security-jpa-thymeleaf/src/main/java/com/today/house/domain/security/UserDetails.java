package com.today.house.domain.security;

import com.fasterxml.jackson.annotation.JsonIgnore;
import com.fasterxml.jackson.annotation.JsonView;
import com.today.house.code.CrudTypeCd;
import com.today.house.code.UseCd;
import com.today.house.domain.base.AdmBase;
import com.today.house.model.security.AuthDepthDetail;
    ...
    public Collection<? extends GrantedObjAuthority<List<AuthDetail>>> getAuthorities() {
        ...
        for (AuthDetail auth : ListUtils.emptyIfNull(authDetails)) {
            GrantedObjAuthority<List<AuthDetail>> selectedAuths = Optional.ofNullable(contain.get(auth.getAuthId())).orElseGet(() -> {
                GrantedObjAuthority<List<AuthDetail>> newAuths = new com.today.house.model.security.GrantedObjAuthority<>(auth.getAuthId(), new ArrayList<>());
                contain.put(auth.getAuthId(), newAuths);
                return newAuths;
            });
            selectedAuths.getAuth().add(auth);
        }
        contain.put("ROLE_AUTH", new com.today.house.model.security.GrantedObjAuthority<List<AuthDetail>>("ROLE_AUTH", null));
