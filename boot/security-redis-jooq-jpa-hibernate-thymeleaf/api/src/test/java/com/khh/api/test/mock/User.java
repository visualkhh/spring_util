...
public class User {
	...
	public void 회원정보_수정() throws Exception {
		OmnifitHeader header = OmnifitHeader.builder().serialNo("serial").cponId("phoneNo").userDvcSeq(500).build();
...
//		.andExpect(jsonPath("$.hello", is("world")));
	...
	@Test
	public void 회원정보_생성() throws Exception {
		OmnifitHeader header = OmnifitHeader.builder().serialNo("serial").cponId("phoneNoffff").build();
		UserDvcBase body = UserDvc.builder().genCd("GEC002").build();
		...
		mockMvc.perform(post("/api/user").headers(headers).content(bodyStr).contentType(MediaType.APPLICATION_JSON_UTF8_VALUE).accept(Omnifit2MediaType.OMNIFIT2_V1_JSON_VALUE))
		.andExpect(status().isOk());
//		.andExpect(content().contentType(APPLICATION_JSON));
//		.andExpect(jsonPath("$.hello", is("world")));
	}
