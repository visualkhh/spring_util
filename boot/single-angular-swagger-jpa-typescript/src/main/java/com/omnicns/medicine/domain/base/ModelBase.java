...
public class ModelBase {
    ...
    public <S, D> D map(PropertyMap<S, D> propertyMap){
        try {
            Class<D> destinationType = (Class<D>) ReflectionUtil.getDeclaredField(propertyMap,ReflectionUtil.getDeclaredField(PropertyMap.class, "destinationType"));
            return map(destinationType, propertyMap);
        } catch (Exception e) {
            return null;
        }
    }
