...
import com.omnicns.medicine.repository.security.UserDetailsRepository;
...
//		Adm adm = admRepository.findByAdmLginId(admLginId);
//		Optional.ofNullable(adm).ifPresent(it->{
//			adm.setLginFailCnt(new Integer(Optional.ofNullable(adm.getLginFailCnt()).orElse(new Integer(0)).intValue()+1));
//			admRepository.saveAndFlush(adm);
//		});

//		UserDetails userDetails = userDetailsRepository.findByAdmLginId(admLginId);
//		Optional.ofNullable(userDetails).ifPresent(it->{
...
//		Session s = entityManager.unwrap(Session.class);//((SessionHolder) TransactionSynchronizationManager.getResource(sessionFactory)).getSession();
//		Session s = (Session) entityManager.getDelegate();
//		s.beginTransaction();
//		Integer i = userDetailsRepository.setLginFailCnt(admLginId);
//		Integer i = s.getNamedQuery("UserDetails.setLginFailCnt").setParameter("admLginId",admLginId).executeUpdate();
//		s.getTransaction().commit();
//		s.clear();
		//		return userDetailsRepository.setLginFailCnt(lginId);
//		return i;
	}
}
