...
import {CoreAdm, CoreAuth, CoreCorpGrp, ProjectHttpHeaderTypes, ProjectMediaTypes, UseCd} from '@generate/models';
...
export class AdmModalComponent implements OnInit {
    ...
    // admInfo: Adm;
    private corps: CoreCorpGrp[];
    private auths: CoreAuth[];
    ...
    show(admSeq?: number) {
        ...
        headers = headers.append(ProjectHttpHeaderTypes.ACCEPT, `${ProjectMediaTypes.APPLICATION_JSON}, ${ProjectMediaTypes.LIST_JSON}`);
        this.api.get<CoreCorpGrp[]>('/setups/corps', {headers}).subscribe(it => {
            this.corps = it;
        }, this.api.errorHandler.bind(this.api));
        this.api.get<CoreAuth[]>('/setups/auths', {headers}).subscribe(it => {
            this.auths = it;
        }, this.api.errorHandler.bind(this.api));

        if (!ValidUtil.isNullOrUndefined(admSeq)) {
            this.api.get<CoreAdm>(`/setups/adms/${admSeq}`)
