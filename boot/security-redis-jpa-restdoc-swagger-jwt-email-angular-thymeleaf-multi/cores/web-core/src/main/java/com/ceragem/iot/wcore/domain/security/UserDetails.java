package com.ceragem.iot.wcore.domain.security;
...
import com.ceragem.iot.wcore.model.security.AuthDepthDetail;
    ...
    public Collection<? extends GrantedObjAuthority<List<AuthDetail>>> getAuthorities() {
        ...
        for (AuthDetail auth : ListUtils.emptyIfNull(authDetails)) {
            GrantedObjAuthority<List<AuthDetail>> selectedAuths = Optional.ofNullable(contain.get(auth.getAuthId())).orElseGet(() -> {
                GrantedObjAuthority<List<AuthDetail>> newAuths = new com.ceragem.iot.wcore.model.security.GrantedObjAuthority<>(auth.getAuthId(), new ArrayList<>());
                contain.put(auth.getAuthId(), newAuths);
                return newAuths;
            });
            selectedAuths.getAuth().add(auth);
        }
        contain.put("ROLE_AUTH", new com.ceragem.iot.wcore.model.security.GrantedObjAuthority<List<AuthDetail>>("ROLE_AUTH", null));
